            ...
            for (const id of livestock_id) {
                ...
                try{ 
                    ...
                    for (const reading of readings) {
                        ...
                        if ((reading.alert && !triggeredSMS.current.has(reading.id) && !triggeredAlert.current.has(reading.id))){
    ...
    const handleSendSMS =  async () => {
        ...
        try{
            ...
            if (res.ok) {
                ...
                setSmsError(errData.detail || "Failed to send SMS. Check you connection.")
            }
        } catch (err) {
        ...
        {alertModal && (
            <div style = {s.backdrop} onClick = {closeAlert} >
                <div style = {s.modal} onClick = {e => e.stopPropagation()}> 
                    <div style= {{...s.modalHeader, background : alertModal.type === "flee" ? "#7c2d12": "#1e3a5f",}}>
                        <span style = {s.headerIcon}> 
                            {alertModal.type === "Flee"}
                        </span>
                        <span style={s.headerTitle}> 
                            {alertModal.type === "flee"? "Flee Detected": "Geofence Breach"}
                        </span>
                        <button style = {s.xBtn} onClick = {closeAlert}> X </button>
                    </div>

                    <div style={s.modalBody}> 
                        <p style = {s.bodyText}> 
                            Livestock <strong style = {s.highlight}> # {alertModal.livestockId}</strong> has triggered a <strong style = {s.highlight}>{alertModal.type} </strong> alert.
                        </p>
                        <div style = {s.coordinateBox}> 
                            <span style= {s.coordinateLabel}>Location</span>
                            <span style= {s.coordinateValue}>
                                {alertModal.latitude.toFixed(6)}, {alertModal.longitude.toFixed(6)}
                            </span>
                        </div>
            ...
            <div style = {s.backdrop} onClick = {closeSMS} >
                <div style = {s.modal} onClick = {e => e.stopPropagation()}> 
                    <div style= {{...s.modalHeader, background : "#78350f"}}>
                        ...
                        <button style = {s.xBtn} onClick = {closeSMS}> X </button>
                    </div>
                    ...
                    <div style={s.modalBody}> 
                        {smsSent? (
                            <div style = {s.successBox}> 
                                ...
                                <p style = {{ color: "#4ade80", fontWeight: "bold", marginTop: "0.5rem"}}> 
                            ...
                            <>
                                <p style={s.bodyText}> 
                                    Livestock <strong style = {s.highlight}> #{smsModal.livestockId} </strong>has triggered an alert. Send an SMS now? 
                                </p>
                                <div style = {s.coordinateBox}>
                                    <span style= {s.coordinateLabel}> Location </span>
                                    <span style = {s.coordinateValue}> 
                                        {smsModal.latitude.toFixed(6)}, {smsModal.longitude.toFixed(6)}
                                    </span>
                                </div>
                                ...
                                <div style = {s.btnRow}> 
                                    <button style = {s.cancelBtn} onClick= {closeSMS} disabled = {smsSending}>
                                        Cancel
                                    </button>
                                    <button 
                                        style = {{
...
const s = {
    ...
    coordinateValue: { color: "#e2e8f0", fontSize: "0.85rem", fontFamily: "monospace" },
    label: {
        display:       "block",
        color:         "#94a3b8",
        fontSize:      "0.8rem",
